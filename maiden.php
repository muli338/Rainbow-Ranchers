...
if(isset($_POST['save']))
{
  ...
  $sql="insert into tblmaiden(CategoryName,ProductName,ProductPrice,ProductAmount,ProductSales,ProductCalving,ProductImage)values(:category,:product,:price,:amount,:sales,:calving,:image)";
  ...
  if ($LastInsertId>0) 
  {
    ...
    echo "<script>window.location.href ='maiden.php'</script>";
  }
  else
  {
...
if(isset($_GET['del'])){    
  $cmpid=$_GET['del'];
  $query=mysqli_query($con,"delete from tblmaiden where id='$cmpid'");
            ...
            <div class="card">
              ...
              <div class="table-responsive p-3">
                <table class="table align-items-center table-flush table-hover table-bordered" id="dataTableHover">
                  ...
                  <tbody>
                    <?php
                    $sql="SELECT tblmaiden.id,tblmaiden.CategoryName,tblmaiden.ProductName,tblmaiden.PostingDate,tblmaiden.ProductPrice,tblmaiden.ProductAmount,tblmaiden.ProductSales,tblmaiden.ProductCalving,tblmaiden.ProductImage from tblmaiden ORDER BY id DESC";
                    $query = $dbh -> prepare($sql);
                    $query->execute();
                    $results=$query->fetchAll(PDO::FETCH_OBJ);
                    $cnt=1;
                      ...
                      { 
                        ...
                        <tr>
                          ...
                          <td class=" text-center"><a href="#"  class=" edit_data4" id="<?php echo  ($row->id); ?>" title="click to edit"><i class="mdi mdi-pencil-box-outline" aria-hidden="true"></i></a>
                            ...
                            <a href="maiden.php?del=<?php echo ($row->id);?>" data-toggle="tooltip" data-original-title="Delete" onclick="return confirm('Do you really want to delete?');"> <i class="mdi mdi-delete"></i> </a>
                          </td>
                        </tr>
                        <?php 
                        $cnt=$cnt+1;
